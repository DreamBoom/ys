...
class TabOne : BaseActivity() {
    ...
    private var serviceGuide = mutableListOf<MainImgBean.ServiceguideBean>()
    ...
    private fun initData(){
        ...
        HttpRequestPort.instance.mainImg(userId, object : BaseHttpCallBack(this) {
            ...
            override fun success(data: String) {
                ...
                //设置轮播图
                if(detail.size==0){
                    val image = bean.image as ArrayList<String>
                }
                //设置图标
                ...
                //设置服务指南
                val service = bean.serviceguide
                serviceGuide.clear()
                serviceGuide.addAll(service)
                adapter!!.notifyDataSetChanged()
                serviceMore!!.visibility = View.VISIBLE
            }

            override fun onError(throwable: Throwable, b: Boolean) {
                super.onError(throwable, b)
                //网络加载失败，设置默认图片
                val array = intArrayOf(R.drawable.img_sy_banner)
               // kar!!.setImagesRes(array)
                imgAdapter = ImgAdapter(this@TabOne, detail, R.layout.img_item)
                imgAdapter!!.notifyDataSetChanged()
                im_info!!.adapter = imgAdapter
